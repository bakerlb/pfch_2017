    ...
    if 'Playwright' in staff:
      ...
      for actor in playbill['cast_member']:
        ...
        for actor_name in actor:
          ...
          if actor_name not in dir_counts[staff['Playwright']]:
            dir_counts[staff['Playwright']][actor_name] = 0

          # increment it by one
          dir_counts[staff['Playwright']][actor_name] = dir_counts[staff['Playwright']][actor_name] + 1

print(json.dumps(dir_counts,indent=4))

json.dump(dir_counts,open('playwright_and_actor.json','w'), indent=4)
