  ...
  for staff in playbill['production_staff']:
    ...
    if 'Director' in staff:
      ...
      for actor in playbill['headliner']:
          if actor not in dir_counts[staff['Director']]:
            dir_counts[staff['Director']][actor] = 0

          # increment it by one
          dir_counts[staff['Director']][actor] = dir_counts[staff['Director']][actor] + 1

print(json.dumps(dir_counts,indent=4))

json.dump(dir_counts,open('director_and_headliner.json','w'), indent=4)
